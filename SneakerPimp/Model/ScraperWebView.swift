...
import WebKit
import GameKit // random number generator
import Combine

/// WKWebView subclass which generates a random userAgent
///
...
class ScraperWebView: WKWebView, ObservableObject {
    ...
    private lazy var randomUserAgent: String = {
        ...
        let index = GKRandomSource.sharedRandom().nextInt(upperBound: userAgents.count - 1)
...
extension ScraperWebView: WKNavigationDelegate {
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        webView.evaluateJavaScript("document.documentElement.outerHTML") { (html, error) in
            ...
            if let html = html {
                self.rawHTMLSubject.send(String(describing: html))
            }
        }
    }
}
